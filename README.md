...
pinmix is a tiny browser extension. open a board on pinterest, hit the shuffle button in the corner, and every pin gets saved to a brand new board in a random order. your original board stays exactly as it was
...
1. it reads the board you are looking at
2. it loads every pin on the board, sections included
3. it shuffles them, with an optional seed if you want the same order again later
4. it creates a new board with the same privacy as the original and saves the pins into it one by one
...
src/
  ...
  mix.ts         load pins, shuffle, create the board, save with retries
